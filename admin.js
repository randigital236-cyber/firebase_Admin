// ==================== RND BACKUP SYSTEM v3.0 - COMPLETE FINAL ====================
...
// ==================== DATA PATHS (SIRF AAPKE MAIN FIREBASE KE HISAB SE) ====================
// आपके Main Firebase में ये Nodes हैं:
// users, deposits, withdrawals, usedTransactions, processingTransactions
...
const Logger = {
    ...
    getRecent(limit) {
        return BackupState.logs.slice(0, limit || 100);
    },
...
function updateRestoreAllButton() {
    ...
    if (badge) {
        badge.textContent = (isSuper && !isLive) ? 'UNLOCKED' : 'LOCKED';
        badge.className = 'badge ' + ((isSuper && !isLive) ? 'success' : 'danger');
    }
    
    if (perm) {
        perm.innerHTML = (isSuper && !isLive) ? 
            '<span style="color:var(--secondary);">✓ Super Admin Access</span>' : 
            '<span style="color:var(--danger);">❌ Super Admin Required</span>';
    }
    
    if (warn) {
        warn.innerHTML = (isSuper && !isLive) ? 
            '⚠️ Type <strong>"RESTORE ALL"</strong> to confirm' : 
            '🔒 <strong>Restore All</strong> is locked for security';
    }
}
...
async function searchUser() {
    ...
    if (!query) {
        Utils.showNotification('Enter a search query', 'warning');
        return;
    }
...
async function compareUserData() {
    ...
    if (!uid) {
        Utils.showNotification('Enter a UID', 'warning');
        return;
    }
...
async function compareSingleUser(uid) {
    ...
    try {
        ...
        if (mismatch) {
            html += `<div class="alert alert-warning"><i class="fas fa-triangle-exclamation"></i><div><strong>Mismatch Found!</strong><br>${matched}/${total} paths matched</div></div>`;
        } else {
            html += `<div class="alert alert-success"><i class="fas fa-check-circle"></i><div><strong>Data Matched!</strong><br>All ${total} paths are identical</div></div>`;
        }
        html += `</div>`;
        ...
        for (const path of USER_DATA_PATHS) {
            ...
            const status = main[path] !== null ? 'Present' : 'Missing';
            html += `<div class="compare-row ${isMis ? 'mismatch' : ''}"><span class="label">${path}</span><span class="value" style="${isMis ? 'color:var(--danger);' : 'color:var(--secondary);'}">${status} ${isMis ? '❌' : '✓'}</span></div>`;
            if (isMis && main[path] !== null) {
                html += `<div class="compare-detail" style="margin-left:0;">Main: ${JSON.stringify(main[path], null, 2)}</div>`;
            }
        }
        html += '</div><div class="compare-column backup"><h4><i class="fas fa-shield-halved"></i> Backup Firebase</h4>';
        for (const path of USER_DATA_PATHS) {
            const isMis = !Utils.deepEqual(main[path], backup[path]);
            const status = backup[path] !== null ? 'Present' : 'Missing';
            html += `<div class="compare-row ${isMis ? 'mismatch' : ''}"><span class="label">${path}</span><span class="value" style="${isMis ? 'color:var(--danger);' : 'color:var(--secondary);'}">${status} ${isMis ? '❌' : '✓'}</span></div>`;
            if (isMis && backup[path] !== null) {
                html += `<div class="compare-detail" style="margin-left:0;">Backup: ${JSON.stringify(backup[path], null, 2)}</div>`;
            }
        }
        html += '</div></div>';
        
        if (mismatch) {
            html += `<div style="margin-top:20px;text-align:center;display:flex;gap:12px;justify-content:center;flex-wrap:wrap;">
                <button class="btn btn-success" onclick="syncSingleUser('${uid}')"><i class="fas fa-rotate"></i> Sync Now</button>
                <button class="btn btn-danger" onclick="restoreSingleUser('${uid}')"><i class="fas fa-rotate-left"></i> Restore</button>
            </div>`;
        }
        container.innerHTML = html;
...
async function restoreAllUsers() {
    if (!Utils.isSuperAdmin()) {
        Utils.showNotification('Super Admin required!', 'error');
        return;
    }
    if (Utils.isLiveDomain()) {
        Utils.showNotification('🔒 Disabled on live domain', 'warning', 6000);
        return;
    }
...
async function detectDeletedUsers() {
    try {
        ...
        const main = ms.val() || {};
        const backup = bs.val() || {};
        const mainIds = Object.keys(main);
        ...
        if (added > 0) {
            BackupState.save();
            Logger.add('Deleted Users', 'warning', added + ' new');
        }
        document.getElementById('deletedUsersCount').textContent = BackupState.deletedUsers.length;
        document.getElementById('deletedBadge').textContent = BackupState.deletedUsers.length;
        ...
        console.error('Deleted detection error:', e);
    }
}
...
function refreshDeletedUsers() {
    detectDeletedUsers();
    renderDeletedUsers();
    Utils.showNotification('Refreshed', 'info');
}
...
async function checkBackupStatus() {
    try {
        ...
        document.getElementById('mainDbStatus').innerHTML = '<span class="tag tag-danger"><i class="fas fa-xmark"></i> Offline</span>';
    }
    try {
        ...
        document.getElementById('backupDbStatus').innerHTML = '<span class="tag tag-danger"><i class="fas fa-xmark"></i> Offline</span>';
    }
    ...
    try {
        ...
        if (result.alerts?.length > 0) {
            ...
            result.alerts.forEach(a => {
                html += `<div style="padding:4px 0;color:#fca5a5;">⚠️ ${a}</div>`;
            });
            html += '</div>';
        }
        container.innerHTML = html;
...
async function downloadBackup() {
    ...
    try {
        ...
        if (result.url) {
            ...
            Utils.showNotification('Backup download started!');
            Logger.add('Download Backup', 'success');
        }
    } catch(e) {
        Utils.showNotification('Download failed: ' + e.message, 'error');
        Logger.add('Download Backup', 'failed', e.message);
    } finally {
        btn.disabled = false;
    ...
    try {
        ...
        for (const path of SYNC_PATHS) {
            try {
                ...
                Utils.showNotification('Failed to sync: ' + path, 'warning');
            }
        }
        BackupState.lastSyncTime = new Date().toISOString();
        BackupState.save();
        ...
        Logger.add('Full Sync', 'failed', e.message);
        Utils.showNotification('Full sync failed: ' + e.message, 'error');
    } finally {
        btn.disabled = false;
        btn.innerHTML = '<i class="fas fa-rotate"></i> Full Sync';
    }
}
    ...
    mainDB.ref('users').on('child_removed', async (snap) => {
        ...
        try {
            ...
            if (bs.val()) {
                const du = {
                    uid: uid,
                    name: user?.name || bs.val()?.name || 'Unknown',
                    email: user?.email || bs.val()?.email || 'N/A',
                    deletedAt: new Date().toISOString()
                };
                if (!BackupState.deletedUsers.find(u => u.uid === uid)) {
            ...
            console.error('Deleted listener error:', e);
        }
    });
}
...
function init() {
    ...
    console.log('✅ RND Backup System v3.0 Started');
    console.log('📧 Admin: rkv858810@gmail.com');
